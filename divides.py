...
def rand():
	...
	# new_ls = [488, 447, 444, 429, 382, 331, 319, 206, 153, 114, 91, 33, 31]
	print(new_ls)
...
def outputMini(new_ls,group1,group2,t1,t1h,t1position,t1hposition):
	...
	#求最小单项
	#把离中间值最接近的上下位数记做最小值
	if  new_ls[t1hposition] and t1h - new_ls[t1hposition] < new_ls[t1hposition-1] - t1h:
		minimsize = t1h - new_ls[t1hposition]
		miniposition.append(len(new_ls)-t1hposition)
	else:
		minimsize = new_ls[t1hposition-1] - t1h
		miniposition.append(len(new_ls)-t1hposition+1)#记录单项最小的位置
